...
function parallax_move(){
	...
	if($page_section_query->have_posts()): 
		...
		if('parallax' == $data['nzs_header_options']){
			$script_return .= "\t".'$("#header-option").parallax("50%", "0.3");'."\n";
		}

		while($page_section_query->have_posts()) : $page_section_query->the_post();

			// $parallax_post = get_post(get_post_meta(get_the_ID(), 'nzs_parallax_attach', TRUE)); 

			$script_return .= "\t".'$("#parallax-sections-'.get_the_ID().'").parallax("50%", "0.3");'."\n";
