...
if ( ! function_exists( 'nzs_social_links' ) ){
	...
	function nzs_social_links(){
		...
		if(!empty($twitter)){
			$str .= '<a class="hide-text twitter" href="'.esc_html($twitter).'">'. __('Twitter','framework') .'</a>';
		}

		if(!empty($facebook)){
			$str .= '<a class="hide-text facebook" href="'.esc_html($facebook).'">'. __('Facebook','framework') .'</a>';
		}
		if(!empty($google)){
			$str .= '<a class="hide-text google" href="'.esc_html($google).'">'. __('Google','framework') .'</a>';
		}
		if(!empty($flickr)){
			$str .= '<a class="hide-text flickr" href="'.esc_html($flickr).'">'. __('Flickr','framework') .'</a>';
		}
		if(!empty($linkedin)){
			$str .= '<a class="hide-text linkedin" href="'.esc_html($linkedin).'">'. __('LinkedIn','framework') .'</a>';
		}
		if(!empty($pinterest)){
			$str .= '<a class="hide-text pinterest" href="'.esc_html($pinterest).'">'. __('Pinterest','framework') .'</a>';
		}
		if(!empty($dribbble)){
			$str .= '<a class="hide-text dribbble" href="'.esc_html($dribbble).'">'. __('Dribbble','framework') .'</a>';
		}
		if(!empty($deviantart)){
			$str .= '<a class="hide-text deviantart" href="'.esc_html($deviantart).'">'. __('DeviantArt','framework') .'</a>';
		}

		if(!empty($youtube)){
			$str .= '<a class="hide-text youtube" href="'.esc_html($youtube).'">'. __('Youtube','framework') .'</a>';
		}

		if(!empty($vimeo)){
			$str .= '<a class="hide-text vimeo" href="'.esc_html($vimeo).'">'. __('Vimeo','framework') .'</a>';
		}

		if(!empty($instagram)){
			$str .= '<a class="hide-text instagram" href="'.esc_html($instagram).'">'. __('Instagram','framework') .'</a>';
		}

		if(!empty($email)){
			$str .= '<a class="hide-text email" href="mailto:'.esc_html($email).'">'. __('Email','framework') .'</a>';
		}

		if(!empty($soundcloud)){
			$str .= '<a class="hide-text soundcloud" href="'.esc_html($soundcloud).'">'. __('Soundcloud','framework') .'</a>';
		}

		if(!empty($behance)){
			$str .= '<a class="hide-text behance" href="'.esc_html($behance).'">'. __('Behance','framework') .'</a>';
		}

		if(!empty($ustream)){
			$str .= '<a class="hide-text ustream" href="'.esc_html($ustream).'">'. __('Ustream','framework') .'</a>';
		}

		if(!empty($rss)){
			$str .= '<a class="hide-text rss" href="'.esc_html($rss).'">'. __('rss','framework') .'</a>';
		}
...
function ninezeroseven_wp_scripts() {
	...
	wp_enqueue_style( 'google-fonts', 'http://fonts.googleapis.com/css?family=Open+Sans:400,600|Oswald:400,300,700');
...
include( get_template_directory() . '/admin/zilla-shortcodes/shortcodes.php' );
...
function ninezeroseven_wp_drop_menu(){
	...
	if ( ( $locations = get_nav_menu_locations() ) && isset( $locations['primary'] ) ) {
		...
		foreach((array) $items as $key => $menu_items){
			...
			$menu_option .="\t\t\t".'<option value="'.$drop_nav_url.'">- '.$menu_items->title.'</option>'. "\n";
		}
